	...
	options={ 
			...
			success :function(data) {
				$.each(data, function(i,  basehospitalinfo) {
					if(basehospitalinfo.uull!== null){
						var uull_array=(basehospitalinfo.uull).split(",");
						
						var marker = new BMap.Marker(new BMap.Point(parseInt(uull_array[0]),parseInt(uull_array[1])));  // 创建标注
						var content = "完成病例数："+basehospitalinfo.endCount+"；高危人群数："+basehospitalinfo.dangerCount;
						map.addOverlay(marker);               // 将标注添加到地图中
						addClickHandler(content,marker,basehospitalinfo.name);
					}
