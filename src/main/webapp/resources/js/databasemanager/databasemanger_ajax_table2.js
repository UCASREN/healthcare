/*
 * This file is responsible for table information show
 *  @author xingkong
 * */
var TableEditable2 = function() {
	var handleTable = function() {
		...
		function saveRow(oTable, nRow) {
			...
			oTable.fnUpdate('<a class="delete" href="">Delete</a>', nRow, 4,
					false);
			oTable.fnDraw();
			// 向后台更改数据逻辑
			var aData = oTable.fnGetData(nRow);
			...
			if (nNew) {
				/*
				 * options={ type : "post",//请求方式 url :
				 * "addpath.action",//发送请求地址 dataType : "json", data:{
				 * "path.pid":aData[0], "path.name":aData[1],
				 * "path.startPosition":aData[2], "path.endPosition":aData[3],
				 * "path.startTime":aData[4]+':00',
				 * "path.endTime":aData[5]+':00', "path.part":aData[6] },
				 * success : function(data) { if(nNew) maxId++; alert(data); } }
				 */
				alert("添加加入接口");
				...
				/*
				 * options={ type : "post",//请求方式 url :
				 * "updatepath.action",//发送请求地址 dataType : "json", data:{
				 * "path.pid":aData[0], "path.name":aData[1],
				 * "path.startPosition":aData[2], "path.endPosition":aData[3],
				 * "path.startTime":aData[4], "path.endTime":aData[5],
				 * "path.part":aData[6] }, success : function(data) { if(nNew)
				 * maxId++; alert(data); } }
				 */
				alert("添加更新接口");
			}
			...
			$.getJSON(url, function(data) {
				$.each(data, function(i, database) {
					if (database.databaseid > maxId)
						maxId = database.databaseid;
					oTable.fnAddData([ database.databaseid, database.name,
							database.comments,
							'<a class="edit" href="">Edit</a>',
							'<a class="delete" href="">Delete</a>' ]);
				});
		...
		table.on('click', '.delete', function(e) {
			...
			$(function() {
				 ...
				 * $.getJSON(url, function(data) { if(data=="删除路线成功！"){
				 * oTable.fnDeleteRow(nRow); alert(data); } });
				 */
			});
