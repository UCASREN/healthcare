...
public class MySQLServiceMetaData implements IService {
	...
	//
	public List<TableInfo> getDatabaseInfo(String mysql_url, String mysql_username, String mysql_password,
			String databaseid) {
		List<TableInfo> resultList = new ArrayList<TableInfo>();
		ConnectionFactory connectionFactory = new ConnectionFactory("mysql", mysql_url, mysql_username,
				mysql_password);
	...
	public boolean insertRemoteDB(String mysql_url, String mysql_username, String mysql_password,
		...
		for (String databaseName : map.keySet()) {
			...
			dbUtilNative.execute("insert into HC_DATABASE (DATABASEID,NAME) values(" + databaseId + ",'"
					+ databaseName + "')");
	...
	public Map<String, List<FieldInfo>> getFieldMap(String mysql_url, String mysql_username, String mysql_password,
	...
	// 获取表相关信息
	public Map<String, List<TableInfo>> getTableMap(String mysql_url, String mysql_username, String mysql_password,
			List<String> dbList) {
		ConnectionFactory connectionFactory = new ConnectionFactory("mysql", mysql_url, mysql_username,
				mysql_password);
		DBUtil dbUtil = new DBUtil(connectionFactory.getInstance().getConnection());
	...
	public boolean changeClassification(String classificationid, String newName, String newComments) {
		...
		try {
			...
			if (newComments != null) {
				sql = "update HC_CLASSIFICATION set COMMENTS='" + newComments + "'  where CLASSIFICATIONID=" + classificationid;
				dbUtil.execute(sql);
			}
			return true;
		} catch (Exception e) {
	...
	public boolean changeDatabase(String databaseid, String newName, String newComments,String classification) {
		...
		try {
			...
			if (classification != null) {
				sql = "update HC_DATABASE set SUBJECTCLASSIFICATION='" + classification + "'  where DATABASEID=" + databaseid;
				dbUtil.execute(sql);
			}
			return true;
		} catch (Exception e) {
	...
	public boolean changeTable(String databaseid, String tableid, String newName, String zhcnname, String newComments,
		...
		try {
			if (newName != null) {
				sql = "update HC_TABLE set NAME='" + newName + "'  where DATABASEID=" + databaseid
						+ " and TABLEID=" + tableid;
				dbUtil.execute(sql);
			}
			if (zhcnname != null) {
	...
	public boolean changeField(String fieldid, String databaseid, String tableid, String newName, String newZhcnname,
		...
		try {
			if (newName != null) {
				sql = "update HC_FIELD set NAME='" + newName + "'  where DATABASEID=" + databaseid
			...
			if (newDatadictionary != null) {
				sql = "update HC_FIELD set DATADICTIONARY='" + newDatadictionary + "'   where DATABASEID="
